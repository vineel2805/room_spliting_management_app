...
import { getMemberExpenseBreakdown } from '../services/calculationService';
...
const MemberDetailModal = ({
  ...
  const memberSettlements = settlements.filter(s =>
    s.fromMemberId === member.memberId || s.toMemberId === member.memberId
  );
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center">
      ...
      <div className="relative bg-surface w-full max-w-lg rounded-t-3xl max-h-[85vh] flex flex-col animate-slide-up">
        ...
        <div className="flex-none">
          ...
          {/* Balance Statement */}
          <div className={`mx-5 mb-4 p-4 rounded-card ${member.balance > 0.01 ? 'bg-success-light' : member.balance < -0.01 ? 'bg-error-light' : 'bg-background'
            }`}>
            <p className={`text-sm font-medium ${member.balance > 0.01 ? 'text-success' : member.balance < -0.01 ? 'text-error' : 'text-text-secondary'
              }`}>
              {member.balance > 0.01
        ...
        {/* Scrollable Content */}
        <div className="flex-1 overflow-y-auto min-h-0 pb-8 overscroll-contain">
          {/* Expenses */}
          <div className="px-5">
                ...
                {expenseBreakdown.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-3 bg-background rounded-card">
                    ...
                    <div className="text-right">
                      <p className={`text-sm font-semibold ${item.net > 0.01 ? 'text-success' : item.net < -0.01 ? 'text-error' : 'text-text-muted'
                        }`}>
          ...
          {memberSettlements.length > 0 && (
            <div className="px-5 mt-6">
              <h3 className="text-base font-medium text-text-primary mb-3">Settlement History</h3>
              <div className="space-y-2">
                {memberSettlements.map((item, idx) => {
                  const isPayer = item.fromMemberId === member.memberId;

                  return (
                    <div key={`settlement-${idx}`} className={`flex items-center gap-3 p-3 rounded-card ${isPayer ? 'bg-background' : 'bg-background'
                      }`}>
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center font-semibold text-sm ${isPayer ? 'bg-error/10 text-error' : 'bg-success/10 text-success'
                        }`}>
                        {(isPayer ? item.toMemberName : item.fromMemberName).charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-text-primary">
                          {isPayer ? `Paid to ${item.toMemberName}` : `Received from ${item.fromMemberName}`}
                        </p>
                        <p className="text-xs text-text-muted">
                          {item.settledAt?.toDate ? formatDate(item.settledAt.toDate()) : 'Settled'}
                        </p>
                      </div>
                      <p className={`text-sm font-semibold ${isPayer ? 'text-error' : 'text-success'
                        }`}>
                        {isPayer ? '-' : '+'}₹{item.amount.toLocaleString('en-IN')}
                      </p>
                    </div>
                  );
                })}
